...
(async () => {
  ...
  const counterFilePath = path.join(__dirname, 'opportunity-counter.txt');
  ...
  try {
    ...
    try {
      ...
      await page.evaluate(() => {
        const radio = document.querySelector('input[type="radio"][value="agent"]') as HTMLInputElement;
        if (radio) {
          radio.checked = true;
          ...
          if (container) {
            const button = container.querySelector('a.selectButton') as HTMLElement;
            if (button) {
    ...
    try {
      ...
      try {
        ...
        const resultados = await page.evaluate(() => {
          ...
          function encontrarCampo(dataField: string, inputClass: string): HTMLInputElement | null {
            ...
            const input = container.querySelector(`input.${inputClass}`) as HTMLInputElement | null;
            if (!input) {
          ...
          function preencherCampo(input: HTMLInputElement | null, valor: string) {
            if (!input) return false;
    ...
    try {
      ...
      try {
        ...
        const clicked = await page.evaluate(() => {
          ...
          const salvarPublicarBtn = buttons.find(btn => 
            btn.textContent?.trim() === ' Salvar e publicar ' || 
            btn.textContent?.trim() === 'Salvar e publicar');
          
          if (salvarPublicarBtn) {
            (salvarPublicarBtn as HTMLElement).click();
            return true;
          }
          ...
          if (publishButtons.length > 0) {
            ...
            (publishButtons[publishButtons.length - 1] as HTMLElement).click();
            return true;
          }
          ...
          try {
            ...
            const clicked = await page.evaluate(() => {
              ...
              if (simButtons.length > 0) {
                (simButtons[0] as HTMLElement).click();
                return true;
              }
