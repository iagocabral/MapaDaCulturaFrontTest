...
(async () => {
    ...
    const counterFilePath = path.join(__dirname, '../contadores', 'event-counter.txt');
    ...
    try {
                ...
                try {
                    ...
                    try {
                        ...
                        if (modalBounds) {
                            ...
                            await page.mouse.click(modalBounds.right - 100, modalBounds.bottom - 50);
        ...
        try {
            ...
            if (await subtitleInput.isVisible()) {
                ...
                await page.fill('.field:has-text("Subtítulo") input, input[aria-label*="subtítulo"]', `Subtítulo do Evento Automático #${counter}`);
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    const clicouJS = await page.evaluate(() => {
                        ...
                        const botaoAdicionar = botoes.find(b => (b.textContent && b.textContent.trim().includes('Adicionar')) &&
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (listBoundingBox) {
                                ...
                                await page.mouse.click(listBoundingBox.x + listBoundingBox.width / 2, listBoundingBox.y + 30 // Aproximadamente onde está o primeiro item
                                );
            ...
            catch (e) { // Explicitly type as any
            ...
            try {
                ...
                await page.fill('input[name="description"], input[placeholder*="resumo customizado"], div.create-occurrence__section:last-child input[type="text"]', `Sessão especial do evento #${counter}. Teatro Municipal.`);
        ...
        try {
            ...
            if (await inscricaoInput.isVisible()) {
                ...
                // Abordagem alternativa
                await page.fill('input[id*="registrationInfo"], .field[data-field="registrationInfo"] input', 'Inscrições pelo telefone ou email do evento.');
        ...
        try {
            ...
            await page.evaluate(() => {
                ...
                const simLibrasLabel = librasLabels.find(label => label.textContent?.includes('Sim') &&
                    label.closest('div')?.previousElementSibling?.textContent?.includes('Libras'));
                if (simLibrasLabel) {
                    simLibrasLabel.querySelector('input[type="radio"]').click();
                    return true;
                }
                return false;
            });
            ...
            await page.evaluate(() => {
                ...
                const simAudioLabel = audioLabels.find(label => label.textContent?.includes('Sim') &&
                    label.closest('div')?.previousElementSibling?.textContent?.includes('Áudio'));
                if (simAudioLabel) {
                    simAudioLabel.querySelector('input[type="radio"]').click();
                    return true;
                }
                return false;
            });
